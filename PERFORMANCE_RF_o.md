...
**Comparaison des prix pour un système complet (Émetteur + Récepteur) :**
* **Ton Système Base :** 30€ (PCB ESP32) + 0€ (Téléphone VRX) = **30 €**
* **Ton Système Cible :** 30€ (PCB) + 24€ (Clé CHANEVE) = **54 €**
* **FPV Analogique (Entrée gamme) :** ~30€ (VTX/Cam) + ~70€ (Masque EV800D) = **~100 €**
* **Walksnail Avatar (Numérique) :** ~130€ (VTX/Cam) + ~250€ (Module VRX) = **~380 €**
* **DJI O3 (Numérique) :** ~250€ (Air Unit) + ~550€ (Goggles V2) = **~800 €**

```mermaid
%%{init: {'themeVariables': {'xyChart': {'plotColorPalette': '#2ecc71, #e74c3c'}}}}%%
xychart-beta
    title "Coût du Système Complet (Émetteur + Récepteur) en €"
...
%%{init: {'themeVariables': {'xyChart': {'plotColorPalette': '#2ecc71, #e74c3c'}}}}%%
xychart-beta
    title "Indice de Valeur (Mètres de portée / €) - Plus c'est haut, mieux c'est"
    x-axis ["Ton Projet (Base)", "Ton Projet (Cible)", "Analogique", "Walksnail", "DJI O3"]
    y-axis "Mètres / €" 0 --> 20
    bar [7.6, 17.0, 0, 0, 0]
    bar [0, 0, 10.0, 10.5, 5.0]
```
